...
async function signin(data) {
    try {
       ...
       if(!user){
        throw new AppError('No user found for the givin email', StatusCodes.NOT_FOUND);
       } 
...
async function addRoletoUser(data){
    try {
        ...
        if(!user){
            throw new AppError('No user found for the givin id', StatusCodes.NOT_FOUND);
           } 
        const role = await roleRepo.getRoleByName(data.role);
        if(!role){
            throw new AppError('No user found fot the givin role', StatusCodes.NOT_FOUND);
           } 
        user.addRole(role);
        return user;
    } catch (error) {
